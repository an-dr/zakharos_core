...
class I2cServerNode:
    ...
    def handle(self, req):
        ...
        try:
            if (req.node_cmd == "w"):
                rospy.loginfo("To write [ addr: 0x%x, reg:0x%x, val: 0x%x]" % (req.addr, req.reg, req.reg_value))
                z.i2c.write(req.addr, req.reg, req.reg_value)
                return srv.I2cResponse(0x0AFF)
            elif (req.node_cmd == "r"):
                rospy.loginfo("To read [ addr: 0x%x, reg:0x%x]" % (req.addr, req.reg))
                read_byte = z.i2c.read(req.addr, req.reg)
    ...
    def start(self):
        ...
        self.server = z.ros.get.server(name=self.name, service=srv.I2c, handle=self.handle)
