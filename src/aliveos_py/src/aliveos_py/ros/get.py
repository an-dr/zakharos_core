...
def client(srv_name: str, service, timeout: float = 5):
    """
    ...
    service : Any
    """
    rospy.logdebug("Client \'%s\' is loading..." % srv_name)
    if timeout < 0:
        return None
    if timeout > 0:
        try:
            rospy.loginfo(f"Waiting for the service {srv_name} ({timeout} sec)")
            rospy.wait_for_service(srv_name, timeout)
            rospy.loginfo(f"Service {srv_name} online")
        except rospy.ROSException:
            rospy.logerr(f"Service {srv_name} is offline!")
    c = rospy.ServiceProxy(srv_name, service)
    rospy.logdebug("Client \'%s\' is ready" % srv_name)
    return c
